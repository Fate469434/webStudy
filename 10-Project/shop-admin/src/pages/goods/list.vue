  ...
  <div>
    ...
    <el-card shadow="never" class="border-0">
      ...
      <el-table ref="multipleTableRef" @selection-change="handleSelectionChange" :data="tableData" stripe
        ...
        <el-table-column label="实际销量" width="150" prop="sale_count" align="center" />
        ...
        <el-table-column label="审核状态" width="150" align="center" v-if="searchForm.tab != 'delete'">
          <template #default="{ row }">
            <div class="flex flex-col" v-if="row.ischeck == 0">
              <el-button type="success" size="small" plain :loading="row.checkpassLoading"
                @click="checkpass(row)">审核通过</el-button>
              <el-button class="mt-2 !ml-0" type="danger" size="small" plain :loading="row.checknotpassLoading"
                @click="checknotpass(row)">审核拒绝</el-button>
            </div>
...
const {
  ...
  onGetListSuccess: (res) => {
    tableData.value = res.list.map(o => {
      ...
      o.skusLoading = false
      o.checkpassLoading = false
      o.checknotpassLoading = false
      return o
    })
...
function checkpass(row) {
  ...
  checkGoods(row.id, { ischeck: 1 })
    ...
    .finally(() => {
      row.checkpassLoading = false
    })
}

// 审核不通过
function checknotpass(row) {
  row.checknotpassLoading = true
  checkGoods(row.id, { ischeck: 2 })
    .then(res => {
      notify("该商品未通过审核")
      // getData()
      row.ischeck = 2
